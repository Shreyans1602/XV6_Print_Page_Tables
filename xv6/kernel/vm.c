...
ptprint(pde_t *pgdir)
{
  ...
  //Print Page Directory Address
  cprintf("pgdir 0x%x\n",pgdir);

  int tIteratorOuter = 0;
  int tIteratorInner = 0;
  for(tIteratorOuter = 0; tIteratorOuter < NPDENTRIES ; tIteratorOuter++)
  {
    pde = &pgdir[tIteratorOuter];
    ...
    {
      pgtab = (pte_t*)PTE_ADDR(*pde);
      cprintf("..%d: pde 0x%x pa 0x%x\n",tIteratorOuter,*pde,pgtab);
      
      for(tIteratorInner = 0; tIteratorInner < NPDENTRIES ; tIteratorInner++)
      {
        pte = pgtab;
        ...
        cprintf(".. ..%d: pte 0x%x pa 0x%x\n",tIteratorInner,*pte,PTE_ADDR(*pte));
